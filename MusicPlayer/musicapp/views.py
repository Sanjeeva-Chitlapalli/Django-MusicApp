...
def index(request):
    ...
    # Display Kannada Songs
    songs_kannada = list(Song.objects.filter(language='Kannada').values('id'))
    sliced_ids = [each['id'] for each in songs_kannada][:5]
    indexpage_kannada_songs = Song.objects.filter(id__in=sliced_ids)

    # Display Korean Songs
    songs_korean = list(Song.objects.filter(language='Korean').values('id'))
    sliced_ids = [each['id'] for each in songs_korean][:5]
    indexpage_korean_songs = Song.objects.filter(id__in=sliced_ids)
    ...
    context = {
        ...
        'kannada_songs':indexpage_kannada_songs,
        'korean_songs':indexpage_korean_songs,
...
def kannada_songs(request):
    ...
    kannada_songs = Song.objects.filter(language='Kannada')
    ...
    if query:
        kannada_songs = Song.objects.filter(Q(name__icontains=query)).distinct()
        context = {'kannada_songs': kannada_songs}
        return render(request, 'musicapp/kannada_songs.html', context)

    context = {'kannada_songs':kannada_songs,'last_played':last_played_song}
    return render(request, 'musicapp/kannada_songs.html',context=context) 

def korean_songs(request):

    korean_songs = Song.objects.filter(language='Korean')
    ...
    if query:
        korean_songs = Song.objects.filter(Q(name__icontains=query)).distinct()
        context = {'korean_songs': korean_songs}
        return render(request, 'musicapp/korean_songs.html', context)

    context = {'korean_songs':korean_songs,'last_played':last_played_song}
    return render(request, 'musicapp/korean_songs.html',context=context) 
